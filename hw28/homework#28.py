...
class Person:
    ...
    def greeting(self):
        print(f"\nHello {self.name}.You're {self.age} years old")

    
    def sleep(self):
        print(f"{self.name} is going sleep.\n")
...
class Animal:
    ...
    def eat(self):
        print(f"\n{self.color} {self.name} eats wallnut")

    
    def run(self):
        print(f"{self.name.capitalize()} heard little noice from backside and ran.\n")
...
def main():
    person = Person("Connor", 28)
    person.greeting()
    person.sleep()

    squirrel = Animal("squirrel", 2, "Brown")
    squirrel.eat()
    squirrel.run()
